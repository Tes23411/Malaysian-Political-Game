...
const BillProposalPanel: React.FC<BillProposalPanelProps> = ({ bill, playerParty, partiesMap, electionResults, onVote }) => {
    ...
    const totalSeats = Array.from(partySeatCounts.values()).reduce((a, b) => (a as number) + (b as number), 0) as number;
    ...
    const otherMajorPartiesStances = useMemo(() => {
        return (Array.from(partiesMap.values()) as Party[])
            ...
            .sort((a,b) => ((partySeatCounts.get(b.party.id) as number) || 0) - ((partySeatCounts.get(a.party.id) as number) || 0))
