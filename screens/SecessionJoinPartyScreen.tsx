...
const SecessionJoinPartyScreen: React.FC<SecessionJoinPartyScreenProps> = ({ parties, currentPartyId, affiliation, onSelect, onCancel }) => {
  ...
  const availableParties = parties.filter(p => {
      ...
      if (!p.ethnicityFocus || p.ethnicityFocus === 'Multi-Racial') return true; // Multi-ethnic

      if (p.ethnicityFocus === 'Multi-Racial (Sabah)') {
          return ['Bumiputera Sabah (Muslim)', 'Bumiputera Sabah (Non-Muslim)', 'Sabahan Chinese', 'Multi-Racial (Sabah)'].includes(affiliation.ethnicity);
      }
      if (p.ethnicityFocus === 'Multi-Racial (Sarawak)') {
          return ['Bumiputera Sarawak (Muslim)', 'Bumiputera Sarawak (Non-Muslim)', 'Sarawakian Chinese', 'Multi-Racial (Sarawak)'].includes(affiliation.ethnicity);
      }
      
      const malaySubtypes = ['Malay', 'Bumiputera Sabah (Muslim)', 'Bumiputera Sarawak (Muslim)'];
      if (p.ethnicityFocus === 'Malay' && malaySubtypes.includes(affiliation.ethnicity as any)) {
          return true;
      }
