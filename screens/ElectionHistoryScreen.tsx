...
import { ElectionHistoryEntry, Party, GeoJsonFeature } from '../types';
...
const ElectionHistoryScreen: React.FC<ElectionHistoryScreenProps> = ({ history, partiesMap, totalSeats, onClose, electionHistory, featuresMap }) => {
  ...
  const historicalPartiesMap = useMemo(() => {
      ...
      if (previousElection && previousElection.parties) {
           previousElection.parties.forEach(p => map.set(p.id, { ...(map.get(p.id) || {}), ...p } as Party));
      }
      ...
      if (selectedElection && selectedElection.parties) {
          selectedElection.parties.forEach(p => map.set(p.id, { ...(map.get(p.id) || {}), ...p } as Party));
      }
      ...
      <div className="absolute inset-0 bg-black bg-opacity-75 z-[5000] flex items-center justify-center font-sans p-4">
        <div className="relative w-full h-full max-w-5xl max-h-[95vh]">
           <ElectionResultsPanel
              ...
              totalElectors={selectedElection.totalElectors}
