...
const PartyMergerScreen: React.FC<PartyMergerScreenProps> = ({ playerParty, parties, affiliations, characters, onPropose, onCancel, mode }) => {
  ...
  const { mergeableParties, mergeableAffiliations } = useMemo(() => {
    ...
    // If Player is Multi-Racial (Regional): Only allow merging with that region's entities.
    // If Player is Multi-Ethnic: Can merge with anyone (result remains Multi-Ethnic).
    if (playerParty.ethnicityFocus && playerParty.ethnicityFocus !== 'Multi-Racial') {
        if (playerParty.ethnicityFocus === 'Malay') {
            const malaySubtypes = ['Malay', 'Bumiputera Sabah (Muslim)', 'Bumiputera Sarawak (Muslim)'];
            mParties = mParties.filter(p => p.ethnicityFocus && malaySubtypes.includes(p.ethnicityFocus as any));
        } else if (playerParty.ethnicityFocus === 'Multi-Racial (Sabah)') {
            const sabahSubtypes = ['Bumiputera Sabah (Muslim)', 'Bumiputera Sabah (Non-Muslim)', 'Sabahan Chinese', 'Multi-Racial (Sabah)'];
            mParties = mParties.filter(p => !p.ethnicityFocus || sabahSubtypes.includes(p.ethnicityFocus as any));
        } else if (playerParty.ethnicityFocus === 'Multi-Racial (Sarawak)') {
            const sarawakSubtypes = ['Bumiputera Sarawak (Muslim)', 'Bumiputera Sarawak (Non-Muslim)', 'Sarawakian Chinese', 'Multi-Racial (Sarawak)'];
            mParties = mParties.filter(p => !p.ethnicityFocus || sarawakSubtypes.includes(p.ethnicityFocus as any));
        } else {
    ...
    if (playerParty.ethnicityFocus && playerParty.ethnicityFocus !== 'Multi-Racial') {
        if (playerParty.ethnicityFocus === 'Malay') {
            const malaySubtypes = ['Malay', 'Bumiputera Sabah (Muslim)', 'Bumiputera Sarawak (Muslim)'];
            mAffiliations = mAffiliations.filter(aff => malaySubtypes.includes(aff.ethnicity as any));
        } else if (playerParty.ethnicityFocus === 'Multi-Racial (Sabah)') {
            const sabahSubtypes = ['Bumiputera Sabah (Muslim)', 'Bumiputera Sabah (Non-Muslim)', 'Sabahan Chinese', 'Multi-Racial (Sabah)'];
            mAffiliations = mAffiliations.filter(aff => sabahSubtypes.includes(aff.ethnicity as any));
        } else if (playerParty.ethnicityFocus === 'Multi-Racial (Sarawak)') {
            const sarawakSubtypes = ['Bumiputera Sarawak (Muslim)', 'Bumiputera Sarawak (Non-Muslim)', 'Sarawakian Chinese', 'Multi-Racial (Sarawak)'];
            mAffiliations = mAffiliations.filter(aff => sarawakSubtypes.includes(aff.ethnicity as any));
        } else {
